...
export default class Channel extends Component {
  ...
  async render() {
    this._style = `
      ...
      #content-users-writing-container {
        display: flex;
        flex-direction: row;
        position: relative;
        margin: 0 1rem 0 1rem;
      ...
      .dot {
        ...
        margin-left: 2px;
        padding: 0;
        background: var(--channel-message-text-color);
    ...
    this._template = `
      <div id="wrapper">
        ...
        <div id="container">
          ...
          <div id="content-users-writing-container"></div>
  ...
  async updateWritingMessages(userId, writing) {
    const context = (await import('/scripts/services/context.esm.js')).default;
    const user = await context.getUser(userId)

    if (writing) {
      this.usersWriting.push(user);
    } else {
      const index = this.usersWriting.findIndex((u) => u.id === userId);
      this.usersWriting.splice(index, 1);
    }

    const container = this.shadowRoot.getElementById('content-users-writing-container');
    if (this.usersWriting.length) {
      const content = this.usersWriting.map((u) => `${u.forename} ${u.surname}`).join(',') + 
                      (this.usersWriting.length > 1 ? ' are ' : ' is ') + 'writing';

      if (container.hasChildNodes()) {
        const a = this.shadowRoot.getElementById('users-writing');
        ...
        wave.appendChild(dot1)
        wave.appendChild(dot2)
        wave.appendChild(dot3)
      ...
      container.innerHTML = ''; // Not super optimized
    }
  }
  ...
  async afterLoad() {
    ...
    this.addBusEventListener(`channel-${this.content.id}`, async (message) => {
      await this.updateWritingMessages(message.userId, false);
    ...
    this.addBusEventListener(`message-writing-channel-${this.content.id}`, async (data) => {
      await this.updateWritingMessages(data.userId, data.content.writing);
    });
    ...
    control.addEventListener('writing', (event) => {
      this.bus.dispatchEvent('message-writing', {
        writing: event.detail, // true or false
        channelId: this.content.id,
      });
    })
