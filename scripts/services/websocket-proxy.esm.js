...
class WebsocketProxy {
  async start(user) {
    ...
    this.websocket.addEventListener('onmessage', (message) => {
      ...
      switch (type) {
        ...
        case 'message-writing': {
          this.incomingMessageWriting(data);
          break;
        }
    ...
    this.bus.addEventListener('message-writing', 'wsp', this.outgoingMessageWriting.bind(this))
  ...
  /** 
   * A chat message has started or stopped being written belonging to a subscribed channel;
   * will be sent forward using the event bus to the correct (channel-)
   * view and append it to the content.
   * @param {object} data Content of message (with associated channel and if being written)
   */
   incomingMessageWriting(data) {
    console.log('[websocket-proxy.esm.js::incoming] <- message-writing', data);

    // Push message data forward to correct channel
    this.bus.dispatchEvent(`message-writing-channel-${data.channelId}`, data);
  }
   ...
   * @param {object} data Content of message (with associated channel and user)
   */
   outgoingMessageWriting(data) {
    console.log('[websocket-proxy.esm.js::outgoing] -> message-writing', data);
