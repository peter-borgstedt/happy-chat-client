  ...
  async afterRender() {
    ...
    editor.addEventListener('input', (event) => {
      if (editor.innerHTML.length) {
        if (this.properties.isWriting < Date.now()) {
          this.dispatchEvent(new CustomEvent('writing', { bubbles: false, composed: false, detail: true }));

          const ref = setInterval(() => {
            if (this.properties.isWriting < Date.now()) {
              this.dispatchEvent(new CustomEvent('writing', { bubbles: false, composed: false, detail: false }));
              this.properties.isWriting = 0;
              clearInterval(ref);
            }
          }, 1000)
        }
        ...
        this.properties.isWriting = expiration.getTime();
      } else {
        this.properties.isWriting = 0;
        this.dispatchEvent(new CustomEvent('writing', { bubbles: false, composed: false, detail: false }));
      }
